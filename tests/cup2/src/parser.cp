` sub REMOVE_ME() { vec<FieldValue>:new_with_cap(0); vec<VarName>:new_with_cap(0); };
...
`         TokenKind:Each {
`             index@ += 1;
`             vec<VarName> vars = vec<VarName>:new_with_cap(2);
`             while opt_token(tokens, index, TokenKind:Ident) {
`                 vars.push(new VarName {
...
`                                         vec<u8> temp = vec<u8>:new_with_cap(64);
`                                         temp.join(lhs_str);
`                                         temp.join(rhs_str);
