...
function parse(start, end) {
    ...
    if (op_count === 0) {
        ...
        switch (tokens[start + 1].type) {
            case token_type.LEFT_PAREN:
                expr.kind = expr_type.FN_CALL;
                expr.name = token.value;
                const paren = next_token_of_type(token_type.RIGHT_PAREN, start + 2, tokens.size);
                if (paren === -1) {
                    throw "expected ')' after '('";
                }
                if (start + 2 === paren) {
                    ...
                    expr.args = [];
                    const comma = next_token_of_type(start + 2, token_type.COMMA, paren);
                    let i = start + 2;
                    while (comma !== -1 && comma + 1 < paren) {
                        expr.args.push(parse(tokens, i, comma));
                        i = comma + 1;
                        comma = next_token_of_type(index, token_type.COMMA, paren);
                    }
                    expr.args.push(parse(index, paren));
                }
                return expr;
            default:
                if (token.value[0] === "\"") {
        ...
        if (expr_index == vector.length) {
            ...
            switch (token.type) {
                ...
                case token_type.MOD:
                    ...
                    break;
                // function dec identification todo
                ...
                // variable dec identification todo
...
function generate_expr(expr, last, semicolon, parens) {
    switch (expr.kind) {
        ...
        case expr_type.FN_DEF:
            ...
            code += " " + expr.name + "(";
            generate_block(expr.args, true, true);
            code += "){";
        ...
        case expr_type.ENUM:
            ...
            for (let i = 0; i < expr.body.length; i++) {
                code += expr.body[i].name + " _" + i + ";";
            }
            code += "}" + expr.name + "Union;";
            code += "typedef struct{" + expr.name + "Union u;";
            generate_type("uint");
            code += " type;}" + expr.name + ";";
            break;
